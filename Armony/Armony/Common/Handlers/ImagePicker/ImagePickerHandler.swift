...
public final class ImagePickerHandler: NSObject {
        ...
        return UIAlertAction(title: title, style: .default) { [unowned self] _ in
            ...
            case .photoLibrary, .savedPhotosAlbum:
                PHPhotoLibrary.requestAuthorization { status in
                    ...
                    case .denied:
                        let goSettings = UIAlertAction(title: "Ayarlar", style: .default) { _ in
    ...
    public func present(from sourceView: UIView) {
        ...
        if let action = self.action(for: .savedPhotosAlbum, title: "Fotoğraf Yükle") {
            alertController.addAction(action)
        }

        alertController.addAction(UIAlertAction(title: "İptal".needLocalization, style: .cancel, handler: nil))
