...
final class FilterViewController: UIViewController, ViewController {
    ...
    override func viewDidLoad() {
        ...
        clearFilterLeftButton.configure(
            with: .init(title: "Temizle", titleColor: .white04, titleFont: .lightBody, backgroundColor: .clear)
        )

        if !viewModel.filters.isEmpty {
            clearFilterLeftButton.configure(
                with: .init(title: "Temizle", titleColor: .white, titleFont: .lightBody, backgroundColor: .clear)
            )
