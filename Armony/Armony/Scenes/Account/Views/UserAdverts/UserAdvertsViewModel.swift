...
final class UserAdvertsViewModel: ViewModel {
    ...
    private let advertCreateNotificationService: CreateAdvertNotificationService = .shared
    ...
    func toggleEmptyStateView() {
        if presentation.cards.isEmpty {
            view?.showEmptyStateView(with: .noContent) { [weak self] _ in
                self?.coordinator.selectTab(tab: .createAdvert)
            }
        }
        else {
    ...
    fileprivate func addDidCreateAdvertNotificationToken() {
        advertCreateNotificationService.addNewAdvertCreateHandler { [weak self] _ in
            self?.handleDidCreateAdvert()
        }
    }

    fileprivate func handleDidCreateAdvert() {
...
extension UserAdvertsViewModel: ViewModelLifeCycle {
    ...
    func viewDidLoad() {
        ...
        addDidCreateAdvertNotificationToken()
...
private extension EmptyStatePresentation {
    static var noContent: EmptyStatePresentation = {
        let title = "Yayınladığın ilanlarını bu alanda yönetebilirsin".needLocalization.emptyStateTitleAttributed
        let buttonTitle = "İlan Ver".emptyStateButtonAttributed
