            ...
            safeSync {
                if isEmpty {
                    self?.showEmptyStateView(with: .noContent) { _ in
                        AlertService.show(title: "Video Ekle",
                        ...
                        }), .cancel()], inputPlaceholder: "YouTube linki", textFieldDelegate: self!)
                    }
                }
                else {
...
private extension EmptyStatePresentation {
    static var noContent: EmptyStatePresentation = {
        let title = "YouTube videolarını ekleyerek performanslarını sergileyebilirsin".needLocalization.emptyStateTitleAttributed
        let buttonTitle = "Video ekle".emptyStateButtonAttributed
